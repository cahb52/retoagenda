...
Router.get('/demo', function(req, res) {
  Usuarios.find({user: req.query.user}).count({}, function(err, count) {
    if(count>0){ 
        res.send("Utilice los siguientes datos: </br>usuario: demo | password:123456 </br>usuario: carlos | password:123456") 
    }else{
      Eventos.find({}).count({}, function(err, count) {
        if(count>0){
          ...
          if(err){
            ...
            console.log("Información de eventos reinicida")
          }
        })
      }
    })
...
Router.post('/login', function(req, res) {
    ...
    sess = req.session;
    Usuarios.find({user: user}).count({}, function(err, count) { //aca verificamos si el usuario esta registrado
        if (err) {
            res.status(500)
            res.json(err) 
        }else{
          if(count == 1){ 
            Usuarios.find({user: user, password: password }).count({}, function(err, count) { 
                if (err) {
                    res.status(500) //Retornamos el estatus de error
                    res.json(err) //Retornamos un error
                }else{
                  if(count == 1){ //Si el conteo es igual a 1 enviar Validado
